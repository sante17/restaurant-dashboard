...
export default function AnalitichePage() {
 ...
 if (loading) return <div className="flex items-center justify-center h-64"><p className="text-[#a8a29e]">Caricamento analitiche...</p></div>;
 ...
 <p className="text-[#a8a29e] mt-1 text-sm">Insights sui tuoi clienti e prenotazioni</p>
 </div>
 <button onClick={loadData} className="bg-[#c2410c] hover:bg-[#9a3412] w-full sm:w-auto px-4 py-2 text-white text-sm font-medium rounded-lg">Aggiorna</button>
 </div>
 ...
 <div className="bg-white rounded-xl border border-[#e8e0d8] p-4">
 <h3 className="text-sm font-semibold text-[#1c1917] mb-3">Orari piu richiesti</h3>
 {topHours.length > 0 ? (
 <div className="space-y-2">
 ...
 <div>
 <span className="text-[#d6cfc7]">Visite</span>
 <p className="font-semibold text-[#1c1917]">{c.totalBookings}</p>
 </div>
 <div>
 <span className="text-[#d6cfc7]">Media pers.</span>
 <p className="font-semibold text-[#1c1917]">{c.avgPersone}</p>
 </div>
 <div>
 <span className="text-[#d6cfc7]">Ultima</span>
 <p className="font-semibold text-[#1c1917]">{formatDate(c.lastBooking)}</p>
 </div>
 </div>
 </div>
 ))}
 ...
 <th className="text-left text-xs font-semibold text-[#a8a29e] uppercase px-4 py-3">Prima visita</th>
 ...
 <td className="px-4 py-3 text-sm text-[#78716c]">{formatDate(c.firstBooking)}</td>
