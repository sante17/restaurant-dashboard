    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">{restaurants.length} ristoranti registrati</p>
        </div>
        <button
      ...
      <div className="space-y-4">
        {restaurants.map((r) => (
          <div key={r.id} className={"bg-white rounded-xl border border-gray-200 p-6 " + (!r.is_active ? "opacity-60" : "")}>
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm text-gray-600">
                  <div>
                    <span className="text-gray-400">Slug:</span> {r.slug}
                  </div>
                  <div>
                    <span className="text-gray-400">Indirizzo:</span> {r.address || "---"}
                  </div>
                  <div>
                    <span className="text-gray-400">Telefono:</span> {r.phone || "---"}
                  </div>
                  <div>
                    <span className="text-gray-400">Creato:</span> {formatDate(r.created_at)}
                  </div>
                </div>
                <div className="flex gap-4 mt-2 text-xs">
                  <span className={r.vapi_assistant_id ? "text-green-600" : "text-gray-400"}>
                    {r.vapi_assistant_id ? "Vapi configurato" : "Vapi non configurato"}
                  </span>
                  <span className={r.google_sheet_id ? "text-green-600" : "text-gray-400"}>
              ...
              <div className="flex gap-2 ml-4">
                <button
                  ...
                  className={"px-3 py-1.5 text-sm font-medium rounded-lg transition-colors " + (r.is_active ? "text-amber-600 hover:bg-amber-50 border border-amber-200" : "text-green-600 hover:bg-green-50 border border-green-200")}
                >
        ...
        {restaurants.length === 0 && (
          <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
            <p className="text-gray-500">Nessun ristorante registrato. Creane uno!</p>
          </div>
        )}
      </div>
    </div>
