    ...
    <div>
      {newBookingsCount > 0 && (
        <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-center justify-between gap-2">
          <span className="text-sm text-blue-700">{newBookingsCount === 1 ? "1 nuova prenotazione" : newBookingsCount + " nuove prenotazioni"}</span>
          <button onClick={refreshData} className="px-3 py-1 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 whitespace-nowrap">Aggiorna</button>
        </div>
      )}
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          <button onClick={refreshData} disabled={refreshing} className="flex-1 sm:flex-none px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50">
            {refreshing ? "Aggiornamento..." : "Controlla nuove"}
          </button>
          <button onClick={() => openAddForm()} className="flex-1 sm:flex-none px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">
            + Nuova prenotazione
          </button>
        </div>
      </div>
      ...
      {(showAddForm || editingId) && (
        <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-5 mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            <button onClick={editingId ? handleUpdate : handleAdd} className="flex-1 sm:flex-none px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">
              {editingId ? "Salva modifiche" : "Crea prenotazione"}
            </button>
            <button onClick={() => { setShowAddForm(false); setEditingId(null); setSelectedBooking(null); resetForm(); }} className="flex-1 sm:flex-none px-4 py-2 text-gray-600 text-sm rounded-lg hover:bg-gray-100 border border-gray-200">
              Annulla
            </button>
            {editingId && (
              <button onClick={() => handleCancel(editingId)} className="flex-1 sm:flex-none px-4 py-2 text-red-600 text-sm rounded-lg hover:bg-red-50 border border-red-200">
      ...
      {selectedBooking && !editingId && (
        <div className="bg-white rounded-xl border border-blue-200 p-4 sm:p-5 mb-6">
          ...
          <div className="flex gap-2">
            <button onClick={() => openEditForm(selectedBooking)} className="flex-1 sm:flex-none px-3 py-1.5 bg-blue-600 text-white text-xs rounded-lg hover:bg-blue-700">Modifica</button>
            <button onClick={() => handleCancel(selectedBooking.ID)} className="flex-1 sm:flex-none px-3 py-1.5 text-red-600 text-xs rounded-lg hover:bg-red-50 border border-red-200">Cancella</button>
          </div>
        </div>
      )}
          ...
          <tbody>
            {tavoli.map(tavolo => (
              <tr key={tavolo.name} className="border-b border-gray-100 last:border-0">
                <td className="sticky left-0 z-10 bg-white border-r border-gray-200 px-3 sm:px-4 py-3">
                  <div className="text-xs sm:text-sm font-medium text-gray-900">{tavolo.name}</div>
                  <div className="text-xs text-gray-400">{tavolo.seats}p</div>
                </td>
                {HOURS.map((h, i) => {
                    ...
                    return (
                      <td key={h} colSpan={span} className={"px-1 py-1 " + (i === LUNCH_START ? "border-l-2 border-l-gray-300" : "") + (i === DINNER_START ? " border-l-2 border-l-gray-300" : "")}>
                        <button onClick={() => { setSelectedBooking(bk); setEditingId(null); setShowAddForm(false); }}
                          ...
                          <div className={"text-xs truncate " + (selectedBooking?.ID === bk.ID ? "text-blue-100" : "text-blue-600")}>{bk.Persone}p - {bk.OraInizio}</div>
                        </button>
                      </td>
                    );
                  }
            ...
            {getDatePrenotazioni().sort((a, b) => a.OraInizio.localeCompare(b.OraInizio)).map(pr => (
              <div key={pr.ID} className="p-4">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <p className="text-sm font-medium text-gray-900">{pr.Nome}</p>
                    <p className="text-xs text-gray-500">{pr.OraInizio} - {pr.OraFine} | {pr.Tavolo} | {pr.Persone}p</p>
                  </div>
                  <p className="text-xs text-gray-400 font-mono">{pr.ID}</p>
                </div>
                {pr.Telefono && <p className="text-xs text-gray-500 mb-2">Tel: {pr.Telefono}</p>}
                <div className="flex gap-2">
                  <button onClick={() => openEditForm(pr)} className="flex-1 px-3 py-1.5 bg-blue-600 text-white text-xs rounded-lg hover:bg-blue-700">Modifica</button>
                  <button onClick={() => handleCancel(pr.ID)} className="flex-1 px-3 py-1.5 text-red-600 text-xs rounded-lg hover:bg-red-50 border border-red-200">Cancella</button>
                </div>
              </div>
            ))}
          </div>
            ...
            <tbody>
              {getDatePrenotazioni().sort((a, b) => a.OraInizio.localeCompare(b.OraInizio)).map(pr => (
                <tr key={pr.ID} className="border-b border-gray-100 last:border-0">
                  <td className="px-4 py-2 text-xs text-gray-500 font-mono">{pr.ID}</td>
                  <td className="px-4 py-2 text-sm font-medium text-gray-900">{pr.Nome}</td>
                  <td className="px-4 py-2 text-sm text-gray-600">{pr.OraInizio} - {pr.OraFine}</td>
                  <td className="px-4 py-2 text-sm text-gray-600">{pr.Tavolo}</td>
                  <td className="px-4 py-2 text-sm text-gray-600">{pr.Persone}</td>
                  <td className="px-4 py-2 text-sm text-gray-600">{pr.Telefono || "---"}</td>
                  <td className="px-4 py-2 text-right space-x-2">
                    <button onClick={() => openEditForm(pr)} className="text-xs text-blue-600 hover:text-blue-800 font-medium">Modifica</button>
