...
export default function ImpostazioniPage() {
 ...
 <button onClick={saveAll} disabled={saving} className="bg-[#c2410c] hover:bg-[#9a3412] px-5 py-2 text-white text-sm font-medium rounded-lg transition-colors disabled:opacity-50">
 ...
 <input type="text" value={info.name} onChange={(e) => { setInfo({ ...info, name: e.target.value }); setSaved(false); }} className="w-full px-3 py-2 border border-[#d6cfc7] rounded-lg text-sm text-[#1c1917] focus:ring-2 focus:ring-[#c2410c]/30 focus:border-[#c2410c] outline-none" />
 </div>
 <div>
 <label className="block text-sm text-[#78716c] mb-1">Indirizzo</label>
 <input type="text" value={info.address} onChange={(e) => { setInfo({ ...info, address: e.target.value }); setSaved(false); }} className="w-full px-3 py-2 border border-[#d6cfc7] rounded-lg text-sm text-[#1c1917] focus:ring-2 focus:ring-[#c2410c]/30 focus:border-[#c2410c] outline-none" />
 </div>
 <div>
 <label className="block text-sm text-[#78716c] mb-1">Telefono ristorante</label>
 <input type="tel" value={info.phone} onChange={(e) => { setInfo({ ...info, phone: e.target.value }); setSaved(false); }} className="w-full px-3 py-2 border border-[#d6cfc7] rounded-lg text-sm text-[#1c1917] focus:ring-2 focus:ring-[#c2410c]/30 focus:border-[#c2410c] outline-none" />
 </div>
 <div>
 <label className="block text-sm text-[#78716c] mb-1">Telefono trasferimento (per agente AI)</label>
 <input type="tel" value={info.transfer_phone} onChange={(e) => { setInfo({ ...info, transfer_phone: e.target.value }); setSaved(false); }} className="w-full px-3 py-2 border border-[#d6cfc7] rounded-lg text-sm text-[#1c1917] focus:ring-2 focus:ring-[#c2410c]/30 focus:border-[#c2410c] outline-none" />
 </div>
 <div className="col-span-2">
 <label className="block text-sm text-[#78716c] mb-1">Informazioni parcheggio</label>
 <textarea value={info.parking_info} onChange={(e) => { setInfo({ ...info, parking_info: e.target.value }); setSaved(false); }} rows={2} className="w-full px-3 py-2 border border-[#d6cfc7] rounded-lg text-sm text-[#1c1917] focus:ring-2 focus:ring-[#c2410c]/30 focus:border-[#c2410c] outline-none resize-none" />
 </div>
 </div>
 </div>

 <div className="bg-white rounded-xl border border-[#e8e0d8] p-6">
 <h2 className="text-lg font-semibold text-[#1c1917] mb-2">Agente AI Vocale</h2>
 ...
 {syncResult && (
 <div className={"mb-4 p-3 rounded-lg text-sm " + (syncResult.startsWith("Errore") ? "bg-[#fef2f2] border border-red-200 text-red-700" : "bg-[#f0fdf4] border border-green-200 text-green-700")}>
 {syncResult}
 </div>
 )}

 <div className="flex gap-3">
 <button onClick={syncVapi} disabled={syncing} className="bg-[#c2410c] hover:bg-[#9a3412] px-5 py-2 text-white text-sm font-medium rounded-lg transition-colors disabled:opacity-50">
