...
export default function MenuPage() {
  ...
  async function addCategory() {
    ...
    setNewCategoryName(""); setShowAddCategory(false); loadData();
    await syncVapi();
  }
  ...
  async function addItem() {
    ...
    resetItemForm(); setShowAddItem(false); loadData();
    await syncVapi();
  }
  ...
  async function updateItem(id: string) {
    ...
    setEditingItem(null); resetItemForm(); loadData();
    await syncVapi();
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Gestisci categorie e piatti del tuo menu</p>
        </div>
          ...
          {selectedCategory ? (
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <div className="p-4 border-b border-gray-200 flex items-center justify-between">
                <h2 className="text-sm font-semibold text-gray-900">Piatti -- {categories.find((c) => c.id === selectedCategory)?.name}</h2>
                <button onClick={() => { setShowAddItem(!showAddItem); setEditingItem(null); resetItemForm(); }} className="px-3 py-1.5 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">+ Aggiungi piatto</button>
              </div>
                ...
                <div className="p-4 border-b border-gray-200 bg-blue-50">
                  <div className="grid grid-cols-2 gap-4 mb-3">
                    <div>
                      ...
                      <input type="text" value={itemForm.name} onChange={(e) => setItemForm({ ...itemForm, name: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-900" placeholder="es. Risotto ai funghi porcini" />
                    </div>
                    <div>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => editingItem ? updateItem(editingItem) : addItem()} className="px-4 py-1.5 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">{editingItem ? "Aggiorna" : "Aggiungi"}</button>
                    <button onClick={() => { setShowAddItem(false); setEditingItem(null); resetItemForm(); }} className="px-4 py-1.5 text-gray-600 text-sm rounded-lg hover:bg-gray-100">Annulla</button>
                  </div>
                </div>
              )}
              ...
              <div>
                {filteredItems.map((item) => (
                  <div key={item.id} className={"flex items-center justify-between px-4 py-3 border-b border-gray-100 last:border-0 " + (!item.is_available ? "opacity-50" : "")}>
                    <div className="flex-1">
